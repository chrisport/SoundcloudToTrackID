...
def addTags(file):
    try:
        ...
        if "large" in artworkURL:  # ...but we need to replace "large" with "t500x500"...
            artworkURL = artworkURL.replace("large",
                                            "t500x500")  # ...to get a decent sized photo that isn't pixelated for the cover art of the mp3
    ...
    if len(sys.argv) == 2 or len(sys.argv) == 3:  # a link was provided
        ...
        if isValid(SONG_TO_DOWNLOAD):
            if isPlayList(SONG_TO_DOWNLOAD):
                ...
                exit(1)
            downloadSongs(SONG_TO_DOWNLOAD)
    else:
        print(HELP_MESSAGE)
